...
public class MultiTouchController<T> {
		...
		public float getMultiTouchDiameter() {
			if (!diameterIsCalculated) {
				if (!isMultiTouch) {
					...
					final float diamSq = getMultiTouchDiameterSq();
	...
	private void anchorAtThisPositionAndScale() {
		...
		// anisotropic
		// scale.
		final float currScaleInv = 1.0f / (!mCurrXform.updateScale ? 1.0f
	...
	public boolean onTouchEvent(MotionEvent event) {
		try {
			final int pointerCount = multiTouchSupported ? (Integer) m_getPointerCount
					.invoke(event) : 1;
			if (DEBUG) {
			...
			for (int histIdx = 0; histIdx <= histLen; histIdx++) {
				...
				if (!multiTouchSupported || pointerCount == 1) {
					...
					final int numPointers = Math.min(pointerCount,
							MAX_TOUCH_POINTS);
					...
					for (int ptrIdx = 0; ptrIdx < numPointers; ptrIdx++) {
						final int ptrId = (Integer) m_getPointerId.invoke(
								event, ptrIdx);
	...
	private void performDragOrPinch() {
		...
		final float currScale = !mCurrXform.updateScale ? 1.0f
				: mCurrXform.scale == 0.0f ? 1.0f : mCurrXform.scale;
		extractCurrPtInfo();
		final float newPosX = mCurrPtX - startPosX * currScale;
		final float newPosY = mCurrPtY - startPosY * currScale;
		final float newScale = startScaleOverPinchDiam * mCurrPtDiam;
		final float newScaleX = startScaleXOverPinchWidth * mCurrPtWidth;
		final float newScaleY = startScaleYOverPinchHeight * mCurrPtHeight;
		final float newAngle = startAngleMinusPinchAngle + mCurrPtAng;
		...
		final boolean success = objectCanvas.setPositionAndScale(
				selectedObject, mCurrXform, mCurrPt);
		if (!success) {
